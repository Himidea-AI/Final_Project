      ...
      <div className="overflow-x-auto rounded-lg border border-zinc-700 bg-zinc-800">
        <table className="w-full min-w-[640px]">
          <thead className="border-b border-zinc-700 bg-zinc-900/60">
            <tr>
              <th className="p-3 text-left text-xs font-semibold uppercase text-zinc-400">순위</th>
              <th className="p-3 text-left text-xs font-semibold uppercase text-zinc-400">
                행정동
              </th>
              <th className="p-3 text-right text-xs font-semibold uppercase text-zinc-400">점수</th>
          ...
          <tbody>
            {rankings.map((r, i) => {
              ...
              const rowCls = isWinner ? 'bg-amber-500/10' : i < 3 ? 'bg-amber-500/5' : '';
              return (
                <tr
                  key={r.district}
                  className={`border-b border-zinc-700/50 last:border-b-0 ${rowCls}`}
                >
                  <td className="p-3 font-mono text-sm text-zinc-100">{r.rank ?? i + 1}</td>
                  ...
                  <td className="p-3 text-right font-mono text-sm text-zinc-100">
                    {r.score.toFixed(1)}
                  </td>
