      ...
      <div className="mb-3 grid gap-3 md:grid-cols-2">
        <div className="rounded-lg border border-stone-700 bg-stone-800 p-5">
          <div className="flex items-start justify-between">
            <div>
              <div className="text-xs uppercase tracking-widest text-stone-400">종합 전망 점수</div>
          ...
          <div className="mt-3 flex flex-wrap gap-2 text-xs">
            <span className="rounded bg-stone-900/60 px-2 py-1 text-stone-300">
              방향 <span className="font-semibold text-stone-100">{directionKo}</span>
            </span>
            {industryDir && (
              <span className="rounded bg-stone-900/60 px-2 py-1 text-stone-300">
                업종 <span className="font-semibold text-stone-100">{industryDir}</span>
              </span>
            )}
            {changeIx && (
              <span className="rounded bg-stone-900/60 px-2 py-1 text-stone-300">
                변화 <span className="font-semibold text-stone-100">{changeIx}</span>
              </span>
            )}
          </div>
          {tf?.narrative && (
            <p className="mt-4 text-sm leading-relaxed text-stone-300">{tf.narrative}</p>
          )}
        </div>
        ...
        <div
          className={`rounded-lg border p-5 ${
            closure
              ? (CLOSURE_LEVEL_CLS[closure.risk_level] ?? 'border-stone-700 bg-stone-800')
              : 'border-stone-700 bg-stone-800'
          }`}
        >
          <div className="flex items-start justify-between">
            <div>
              <div className="text-xs uppercase tracking-widest text-stone-400">폐업 위험도</div>
              <div className="mt-1 text-4xl font-bold">
                {closure ? Math.round(closure.risk_score * 100) : '—'}
                <span className="ml-1 text-lg text-stone-500">{closure ? '%' : '/100'}</span>
              </div>
            </div>
