...
export function CustomerSegmentCard({ segment }: Props) {
  ...
  return (
    <div className="bg-stone-900/40 border border-stone-800/60 rounded-3xl p-8 space-y-6">
      ...
      {/* 자연어 요약 */}
      <p className="text-[0.8125rem] text-stone-300 leading-relaxed">{segment.profile_summary}</p>
          ...
          <div className="space-y-2">
            {dimensions.map(({ key, value }) => (
              <div key={key} className="flex items-center gap-3">
                ...
                <span className="text-[0.6875rem] font-black text-stone-300 tabular-nums w-12 text-right">
                  {(value * 100).toFixed(1)}%
                </span>
              </div>
            ))}
