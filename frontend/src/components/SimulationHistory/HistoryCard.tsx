...
export function HistoryCard({ item, onOpen, onDelete, onDownloadPdf }: HistoryCardProps) {
  ...
  const signalCls = SIGNAL_CLS[signalKey] ?? 'bg-zinc-700/40 text-zinc-300 border-zinc-600';
  const signalLbl = SIGNAL_LABEL[signalKey] ?? '—';
  const docId = formatDocumentId(item.id);

  return (
    <div className="rounded-lg border border-zinc-700 bg-zinc-800 p-4 transition-colors hover:border-zinc-600">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 text-xs text-zinc-500">
            <span>📅</span>
            <span className="font-mono">{formatWhen(item.created_at)}</span>
            <span className="ml-1 rounded bg-zinc-900/60 px-1.5 py-0.5 text-[10px] font-mono text-amber-400">
              {docId}
            </span>
          </div>
          <div className="mt-2 flex flex-wrap items-center gap-2">
            <span className="text-sm font-semibold text-zinc-100">👤 {item.client_name}</span>
          </div>
          <div className="mt-1 text-xs text-zinc-400">
            🏪 <span className="text-zinc-200">{item.brand_name}</span> —{' '}
            <span className="text-amber-400">{item.district}</span>
            {item.business_type && (
              <span className="ml-1 text-zinc-500">· {item.business_type}</span>
            )}
          </div>
          <div className="mt-2 flex items-center gap-2">
            ...
            {item.ai_verdict_summary && (
              <span className="truncate text-xs text-zinc-400">{item.ai_verdict_summary}</span>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="mt-3 flex gap-2">
        <button
          ...
          className="rounded-md border border-amber-500/60 bg-amber-500/10 px-3 py-1.5 text-xs font-semibold text-amber-400 hover:bg-amber-500/20"
        >
          상세 보기
        </button>
        {onDownloadPdf && (
          <button
            ...
            className="inline-flex items-center gap-1 rounded-md border border-zinc-700 bg-zinc-800 px-3 py-1.5 text-xs font-medium text-zinc-300 hover:bg-zinc-700"
          >
