...
import { Loader2, MapPin, Coffee, AlertTriangle, ShieldCheck, Users, BarChart3, FileText, Play, Activity } from 'lucide-react';

import { motion, AnimatePresence } from 'framer-motion';

const LOADING_STEPS = [
  { p: 10, m: "상권 전문가(Market Analyst)가 지역 기반 데이터를 수집 중입니다...", icon: <BarChart3 className="text-blue-500" /> },
  { p: 35, m: "인구 전문가(Population Analyst)가 유동인구 및 배후세대 추이를 분석 중입니다...", icon: <Users className="text-emerald-500" /> },
  { p: 65, m: "법률 전문가(Legal Advisor)가 상가임대차 법령 및 리스크를 검토 중입니다...", icon: <ShieldCheck className="text-amber-500" /> },
  { p: 85, m: "감독관(Supervisor)이 모든 분석 결과를 취합하여 종합 리포트를 생성 중입니다...", icon: <FileText className="text-indigo-500" /> },
  { p: 95, m: "최종 리포트 인쇄 및 대시보드 시뮬레이션 준비 중...", icon: <Loader2 className="text-gray-400 animate-spin" /> },
];
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-10 pt-32 pb-20">
        ...
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12 bg-[#2c2825] p-6 rounded-3xl border border-[#3a3633] shadow-2xl">
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
            ...
            <p className="text-gray-500 mt-1 font-mono text-xs tracking-widest uppercase">Multi-Agent Simulation Engine</p>
          </motion.div>
          ...
          >
            <div className="flex items-center gap-3 bg-[#1e1b18] px-4 py-2.5 rounded-2xl border border-[#3a3633] focus-within:border-indigo-500/50 transition-colors">
              ...
              <div className="flex flex-col">
                <span className="text-[10px] text-gray-500 font-bold uppercase tracking-tighter">District</span>
                <input
                  value={district}
          ...
          {loading ? (
            ...
            >
                ...
                <div className="flex justify-between items-end mb-6">
                  <div className="flex items-center gap-4">
                    ...
                    <div className="text-left">
                      <p className="text-xs font-bold text-indigo-400 uppercase tracking-[0.2em] mb-1 animate-pulse">Processing Simulation</p>
                      <h3 className="text-xl font-black text-white">에이전트 군단 협업 분석 중</h3>
                    </div>
                  </div>
                  <div className="flex flex-col items-end">
                    <span className="text-4xl font-black text-indigo-400 tabular-nums">{Math.floor(progress)}%</span>
                    <span className="text-[10px] text-gray-500 font-mono tracking-tighter">SYNCING DATA...</span>
                  </div>
                </div>
              ...
              >
                ...
                <p className="text-gray-300 font-bold leading-relaxed text-lg">
                  {statusMessage}
                </p>
                <div className="flex justify-center gap-2 mt-6">
                  {LOADING_STEPS.map((_, i) => (
                    <div
                      key={i}
                      className={`h-1 rounded-full transition-all duration-500 ${i <= currentStepIdx ? "w-8 bg-indigo-500" : "w-4 bg-[#3a3633]"
                        }`}
                    />
                  ))}
                </div>
                    ...
                    {result.legal_risks?.map((risk, idx) => (
                      ...
                      >
                        ...
                        <div>
                          ...
                          <p className="text-sm font-bold text-gray-300 leading-snug">{risk.detail}</p>
                        </div>
                      </motion.div>
                    ))}
                  </div>
            ...
            >
              <div className="absolute inset-0 opacity-10 pointer-events-none"
                style={{ backgroundImage: `url("data:image/svg+xml,%3Csvg width='60' height='60' viewBox='0 0 60 60' xmlns='http://www.w3.org/2000/svg'%3E%3Cg fill='none' fill-rule='evenodd'%3E%3Cg fill='%23818cf8' fill-opacity='0.1'%3E%3Cpath d='M36 34v-4h-2v4h-4v2h4v4h2v-4h4v-2h-4zm0-30V0h-2v4h-4v2h4v4h2V6h4V4h-4zM6 34v-4H4v4H0v2h4v4h2v-4h4v-2H6zM6 4V0H4v4H0v2h4v4h2V6h4V4H6z'/%3E%3C/g%3E%3C/g%3E%3C/svg%3E")` }} />
              ...
              <h2 className="text-3xl font-black text-white mb-4 tracking-tight">Ready for <span className="text-indigo-400 italic">Insights?</span></h2>
              <p className="text-gray-500 font-medium max-w-sm text-center leading-relaxed">
                상단 제어 패널에서 분석할 지역과 브랜드를 설정하세요.<br />
