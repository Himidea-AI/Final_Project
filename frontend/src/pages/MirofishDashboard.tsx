...
const INITIAL_LOGS = [
  {
    ...
    agent: 'System',
    type: 'THINKING',
    message: "타겟이 '망원1동'입니다. 미로피쉬 로컬 트윈을 가동합니다.",
  },
  {
    id: 2,
    time: '14:19:58',
    agent: 'MiroFishAdapter',
    type: 'THINKING',
    message: '망원1동 디지털 트윈 시뮬레이션 환경(Ollama-Phi3) 초기화...',
  },
];
...
const AGENT_RESPONSES = [
  ...
  },
  {
    ...
    type: 'SUCCESS',
    message:
      '[발화] 즉시 아메리카노 500원 할인 이벤트와 쿠폰 폭격으로 골목 유동인구를 모두 빨아들이겠습니다.',
  },
  {
    id: 5,
    time: '14:20:03',
  ...
  },
  {
    ...
    agent: '개인감성매장 점주',
    type: 'SUCCESS',
    message: '[발화] 단골들에게 무료 디저트를 제공하고 인스타그램 광고 예산을 2배로 늘리겠습니다.',
  },
  {
    id: 7,
    time: '14:20:10',
    agent: 'System',
    type: 'SUCCESS',
    message: '경쟁/카니발리제이션 분석 노드 처리 완료.',
  },
];
        ...
        <div className="col-span-1 bg-slate-800 rounded-xl p-6 border border-slate-700 shadow-xl">
          ...
          <div className="space-y-4">
            ...
            <div className="bg-slate-900 rounded p-4 border border-slate-800">
              ...
              <p
                className={`text-xl font-bold ${threatLevel.includes('High') ? 'text-red-400' : 'text-slate-300'}`}
              >
                {threatLevel}
              </p>
            </div>
        ...
        <div className="col-span-2 bg-slate-950 rounded-xl p-0 border border-slate-700 shadow-xl overflow-hidden flex flex-col">
          ...
          <div className="p-6 h-[500px] overflow-y-auto space-y-4 font-mono text-sm">
            {logs.map((log) => (
              <div
                key={log.id}
                className="flex flex-col p-3 rounded bg-slate-900 border-l-4 border-slate-700 hover:bg-slate-800 transition-colors"
              >
                ...
                <div className="flex">
                  <span
                    className={`mr-2 px-2 py-0.5 rounded text-xs font-bold ${
                      log.type === 'THINKING'
                  ...
                  <span
                    className={`${log.type === 'SUCCESS' && log.message.includes('발화') ? 'text-green-300 font-medium' : 'text-slate-300'}`}
                  >
                    {log.message}
                  </span>
                </div>
