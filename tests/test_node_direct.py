...
async def test_node_logic():
    ...
    try:
        ...
        market_tool.get_population_trends = AsyncMock(return_value={
            "current_pop": 150000, "summary": "최근 1년 유동인구 15% 증가 추세", "qoq_growth": 3.5, "yoy_growth": 15.2, "trend": "성장"
        })
        market_tool.get_commercial_insights = AsyncMock(return_value={
            "statistical_summary": "건당 평균 결제액 12,500원, 20대 여성 고객 주도, 전년 대비 매출 8.5% 증가", "yoy_growth": 8.5, "qoq_growth": 1.2, "trend": "성장"
        })
        market_tool.get_competitor_stats = AsyncMock(return_value={
            "summary": "반경 500m 내 경쟁 업체 12개 존재, 밀집도 높음", "competitor_count": 12, "density_level": "HIGH"
        })
        market_tool.get_rent_insight = AsyncMock(return_value={
            "summary": "평당 임대료 18만원 수준으로 마포구 평균 대비 높음", "avg_rent_3_3m2": 180000, "affordability": "CAUTION"
        })
        ...
        print("-" * 50)
        print(f"2. analysis_metrics (숫자 지표):")
