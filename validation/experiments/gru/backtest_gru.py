"""
GRU 매출 예측 백테스팅 — 2024년 마포구 매출 예측 정확도 검증

LSTM backtest_revenue.py 구조를 그대로 따르되, 아래 항목을 GRU에 맞게 변경:
- LSTMForecaster → GRUForecaster
- window_size=6 → 4 (GRU train config와 일치)
- hidden_size=256 → 128 (GRU train config와 일치)
- 가중치/스케일러 경로: gru_forecast/weights/
- 결과를 validation/results/gru_backtest_results.csv 로 저장
  (LSTM backtest_revenue.py는 print만 있고 저장 없었음 — GRU에서 추가)

담당: B2 — 수지니
참조: validation/backtest_revenue.py (LSTM 구조 동일)
"""
...
# 프로젝트 루트: 이 파일 위치 기준 3단계 상위 (experiments/gru/ → experiments/ → validation/ → root)
PROJECT_ROOT = Path(__file__).resolve().parents[3]

# 결과 저장 경로 — validation/results/gru_backtest_results.csv
RESULTS_DIR = PROJECT_ROOT / "validation" / "results"
RESULTS_CSV = RESULTS_DIR / "gru_backtest_results.csv"

# DB 연결 URL — 환경변수에서 로드 (LSTM backtest_revenue.py와 동일 방식)
...
# LSTM backtest_revenue.py와 동일한 매핑 사용 (데이터 포맷이 같으므로)
# ---------------------------------------------------------------------------
_SALES_COL_MAP = {
...
def _load_sales_data() -> pd.DataFrame:
    ...
    LSTM backtest_revenue.py의 동일 함수와 구조 동일 — 데이터 소스가 같으므로.
    """
    # DB 우선 시도
    try:
...
def _predict_revenue_gru(
    ...
    model,
    test_year: int = 2024,
    window_size: int = 4,   # GRU train config 일치: 4 (LSTM은 6이었음)
    n_steps: int = 4,
) -> float | None:
    ...
    - LSTM backtest_revenue.py의 _predict_revenue() 와 동일한 방식
...
def backtest_gru(test_year: int = 2024) -> dict:
    ...
    # GRU 전용 모듈 import
    from models.gru_forecast.model import WEIGHTS_DIR, GRUForecaster
    from models.gru_forecast.train import load_scalers
    ...
    if not weights_path.exists():
        return {
            "test_year": test_year,
            "error": f"GRU 가중치 파일 없음: {weights_path}\n먼저 finetune을 실행하세요.",
        }

    if not scalers_path.exists():
        return {
            "test_year": test_year,
            "error": f"GRU 스케일러 파일 없음: {scalers_path}",
        }
    ...
    # GRU 모델 초기화 및 가중치 로드
    # hidden_size=128: GRU train config와 일치 (LSTM은 256)
    model = GRUForecaster(
    ...
    model.to(device)
    model.eval()
    logger.info("GRU 모델 로드 완료 (input_size=%d, hidden_size=128)", input_size)

    # -----------------------------------------------------------------------
    # 3. 시계열 데이터 구성 (전체 기간 — _predict_revenue_gru 내부에서 cutoff 적용)
    # -----------------------------------------------------------------------
    ...
    # -----------------------------------------------------------------------
    # 4. 각 동×업종에 대해 GRU 예측 수행 (데이터 누수 방지)
    # -----------------------------------------------------------------------
    ...
    for _, row in actual_agg.iterrows():
        ...
        pred_sales = _predict_revenue_gru(
            ...
            model=model,
            test_year=test_year,
            window_size=4,  # GRU 전용: 4 (LSTM 6과 다름)
        )
...
def save_results_csv(result: dict) -> Path:
    ...
    GRU 백테스트에서는 결과 재현성과 LSTM과의 비교 분석을 위해 CSV 저장 추가.
...
def print_report_gru(result: dict) -> None:
    """백테스트 결과를 콘솔에 포맷팅하여 출력한다.

    LSTM backtest_revenue.py의 print_report() 와 동일한 형식.
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="GRU 매출 예측 백테스팅")
    parser.add_argument(
        "--year",
        type=int,
        default=2024,
