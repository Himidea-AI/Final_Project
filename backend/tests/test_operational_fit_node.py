"""
operational_fit_node (LangGraph 에이전트 래퍼) 테스트

에이전트 노드의 책임:
    1. score_all_districts 호출
    2. AgentAttribution 생성
    3. analysis_results["operational_fit_result"] 구조로 저장
    4. operational_fit_results 필드로 16동 전체 결과 전달
...
@pytest.mark.asyncio
async def test_operational_fit_node_success_with_mock() -> None:
    """정상 경로 — score_all_districts mock 결과로 attribution·evidence 생성 확인."""
    mock_results = {
        "서교동": {
            "operational_fit_score": 95.7,
            "subway_sub": 56.7,
            "bus_sub": 100.0,
            "fclty_sub": 100.0,
            "evidence": {
    ...
    with patch("src.agents.nodes.operational_fit.score_all_districts", return_value=mock_results):
        result = await operational_fit_node(state)

    # 필드 구조 검증
    assert result["current_agent"] == "operational_fit"
    assert result["operational_fit_results"] == mock_results
    assert "analysis_results" in result
    assert "operational_fit_result" in result["analysis_results"]

    blob = result["analysis_results"]["operational_fit_result"]
    ...
    assert attr["id"] == "operational_fit"
...
@pytest.mark.asyncio
async def test_operational_fit_node_empty_results_graceful_fail() -> None:
    """빈 결과 반환 시 confidence 0, verdict '데이터 없음'."""
    state = {"winner_district": "서교동", "target_district": "서교동"}

    with patch("src.agents.nodes.operational_fit.score_all_districts", return_value={}):
        result = await operational_fit_node(state)

    attr = result["analysis_results"]["operational_fit_result"]["agent_attribution"]
    assert attr["confidence"] == 0.0
    assert "없음" in attr["verdict"]
    assert result["operational_fit_results"] == {}


@pytest.mark.asyncio
async def test_operational_fit_node_exception_graceful_fail() -> None:
    ...
    with patch("src.agents.nodes.operational_fit.score_all_districts", side_effect=_raise):
        result = await operational_fit_node(state)
...
@pytest.mark.asyncio
async def test_operational_fit_node_uses_target_district_when_no_winner() -> None:
    """winner_district 없으면 target_district 사용."""
    mock_results = {
        "공덕동": {
            "operational_fit_score": 47.8,
            "subway_sub": 47.9,
            "bus_sub": 49.1,
            "fclty_sub": 46.3,
...
def test_operational_fit_node_real_db() -> None:
    """실DB로 에이전트 실행 — 16동 전체 계산 + attribution 생성 확인."""
    state = {"winner_district": "서교동", "target_district": "서교동"}
    result = asyncio.run(operational_fit_node(state))

    assert len(result["operational_fit_results"]) == 16
    attr = result["analysis_results"]["operational_fit_result"]["agent_attribution"]
    assert attr["confidence"] == 0.85
    assert "서교동" in attr["verdict"]

    # 기존 Phase B 검증(R²=0.55)과 분포 일치 확인
    scores = [r["operational_fit_score"] for r in result["operational_fit_results"].values()]
