"""
operational_fit_scorer — 후보 행정동의 교통·집객 접근성 점수 서비스
...
출력:
- 각 동별 {operational_fit_score, subway_sub, bus_sub, fclty_sub, evidence}

네이밍 주의:
- legal.py의 accessibility_law(장애인편의증진법)와 구분하기 위해 operational_fit_* 접두를 사용한다.
"""
...
# TCN 피처 → operational_fit 서브점수 매핑
...
def calibrate_weights_from_shap(
    ...
    logger.info(
        f"[operational_fit] SHAP 캘리브레이션 — subway={subway_prior:.2f}(prior) "
...
async def score_all_districts(
...
) -> dict[str, OperationalFitResult]:
    """마포 16동의 operational_fit_score 일괄 계산.
    ...
    results: dict[str, OperationalFitResult] = {}
    for dong in MAPO_DONG_CODES:
        ...
        results[dong] = OperationalFitResult(
            operational_fit_score=round(final, 1),
    ...
    score_vals = [r["operational_fit_score"] for r in results.values()]
    logger.info(
        f"[operational_fit] 16동 점수 범위 {min(score_vals):.1f}~{max(score_vals):.1f} "
