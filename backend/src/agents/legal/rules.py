...
def rule_food_hygiene(business_type: str) -> dict:
    ...
    if biz in ("카페", "음식점"):
        ...
        level = "caution"
        summary = (
            f"{biz or '해당 업종'}은 즉석조리/도시락 판매 등 식품접객 행위가 포함될 경우에 한해 "
            "식품위생법 영업신고가 필요할 수 있습니다."
        )
        actions = [
            "즉석조리·도시락 판매 등 식품접객 행위 여부 확인",
            "필요 시 관할 구청 위생과에 영업신고 (식품위생법 제37조)",
        ]
        recommendation = _format_recommendation(
            ["식품위생법 제37조"],
            actions,
...
def rule_sewage(business_type: str) -> dict:
    """음식점은 그리스트랩(유분분리기) 의무 → caution. 그 외 → safe."""
    biz = _normalize_biz(business_type)
        ...
        articles = [
            {
                ...
                "content": (
                    ...
                    "배수설비를 설치하여야 한다. 음식점 등 유분이 다량 발생하는 영업의 경우 "
                    "유분분리기 등 개인하수처리시설 설치가 의무이다."
                ),
            }
        ]
    else:
