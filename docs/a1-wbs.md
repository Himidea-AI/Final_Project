...
### 딥러닝 예측 모델 (IM3-110 하위, B2 수지니 공동)
...
### 4주차 예정 (04/21 ~ 04/25) — 분석 노드 구현

| 티켓 | 작업 | 비고 |
|------|------|------|
| IM3-57 | population_node — 생활인구 API 연동 | B1 에이전트 연결 |
| IM3-58 | demographics_node — SGIS 인구/가구 연동 | B1 에이전트 연결 |
| IM3-59 | cost_node — 임대료/실거래가 연동 | B1 에이전트 연결 |
| IM3-60 | commercial_node + trend_node | 골목상권 + 트렌드 |
| IM3-61 | 데이터 서비스 통합 테스트 | 전체 노드 검증 |
| IM3-173 | store_info vs kakao_store 교차 검증 | 브랜드 정합성 |
...
| 티켓 | 작업 | 비고 |
|------|------|------|
| IM3-175 | 발표 데모용 데이터 검증 | 16개 동 무결성 QA |
| IM3-176 | 문서화 — ERD/스키마/데이터사전 업데이트 | kakao_store 반영 |
| IM3-125 | Chronos 베이스라인 테스트 (선택) | P3, 시간 여유 시 |
