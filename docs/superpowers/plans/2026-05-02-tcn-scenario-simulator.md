...
- [ ] **Step 1: perturb_and_predict 함수 추가**
...
- [ ] **Step 2: 실행 가능 여부 확인 (가중치 없어도 import만 확인)**
...
- [ ] **Step 4: Commit (JSON 파일은 .gitignore에 있으므로 스크립트만)**
...
- [ ] **Step 1: main.py에서 customer_segment 라우터 등록 패턴 확인**
...
- [ ] **Step 2: 같은 패턴으로 sensitivity 라우터 등록**
...
- [ ] `python -m pytest tests/test_sensitivity.py -v` → 11 passed
- [ ] `python -m models.tcn_forecast.sensitivity` 배치 실행 성공
- [ ] `GET /predict/sensitivity?dong_code=11440530&industry_code=CS100001` → 200 OK
- [ ] `GET /predict/sensitivity?dong_code=99999999&industry_code=CS999999` → 404
- [ ] `ruff check backend/src/api/sensitivity.py models/tcn_forecast/sensitivity.py` → 오류 없음
- [ ] C1에게 Task 8 인터페이스 계약 전달
